...
async def set_chat_photo(client: Client, message: Message):
    ...
    if not (can_change_admin or can_change_member):
        await message.edit_text("lu bukan Admin Bambang")
    if message.reply_to_message:
...
async def member_ban(client: Client, message: Message):
    ...
    if not bot.can_restrict_members:
        return await rd.edit("Saya bukan PETINGGI disini")
    if not user_id:
...
**Kicked By:** {message.from_user.mention if message.from_user else 'Anon'}"""
    ...
    try:
        ...
        return await rd.edit("**Lu bukan petingi ler**")
...
async def demote(client: Client, message: Message):
    user_id = await extract_user(message)
    rd = await message.edit_text("`Sabar ya ler`")
    if not user_id:
        return await rd.edit("pengguna tidak ditemukan")
    if user_id == client.me.id:
...
add_command_help(
    "Admin",
    [
        [f"{cmds}ban [reply/username/userid]", "Ban Jamet."],
        [f"{cmds}unban [reply/username/userid]", "Unban Jamet.",],
        [f"{cmds}kick [reply/username/userid]", "kick Jamet dari group."],
        [f"{cmds}promote `or` .fullpromote","Promote Jadi PETINGGI.",],
        [f"{cmds}demote", "Demote pengguna."],
        [f"{cmds}mute [reply/username/userid]","Mute Jamet.",],
        [f"{cmds}unmute [reply/username/userid]","Unmute Jamet.",],
        [f"{cmds}pin [reply]","Buat Pin Pesan.",],
        [f"{cmds}unpin [reply]","Buat lepas Pin Pesan.",],
        [f"{cmds}setgpic [reply ke Gambar]","buat jadi PP GC",],
    ],
)
