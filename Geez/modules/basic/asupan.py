...
async def ppanime(client, message):
    yanto = await message.reply("🔎 `Search PP Wibu...`")
    message.from_user.first_name
    message.from_user.id
    await message.reply_photo(
        choice(
        ...
        False,
        caption=f"📌 Nih PP bau bawang lu",
    )
...
add_command_help(
    "Asupan",[
        ...
        [f"{cmds}ayang", "Mencari Foto ayang lu /nNote: Modul ini buat cwo Nolep."],
        [f"{cmds}ppcp", "Mencari PP CP Random."],
        [f"{cmds}bokep", "Buat nyari Bahan Coli."],
        [f"{cmds}ppanime", "Mencari Foto PP CP Wibu."],
    ],
)
